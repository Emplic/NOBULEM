...
-- instance class (Rewritten for advanced features)
...
function InstanceObject:Render()
	...
	if visible then
		...
		txt.Font = self.interface.sharedSettings.textFont;
...
local EspInterface = {
	...
	sharedSettings = {
		textSize = 13,
		textFont = 2, -- 0=UI, 1=System, 2=Plex, 3=Monospace
		limitDistance = false,
		maxDistance = 150,
		useTeamColor = false
	},
...
function EspInterface.AddInstance(instance, options)
	...
	if cache[instance] then
		...
		-- Pass interface to InstanceObject
