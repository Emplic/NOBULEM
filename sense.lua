...
local function getFontValue(fontSetting)
    if type(fontSetting) == "number" then
        return fontSetting
    elseif type(fontSetting) == "string" and FONT_MAPPING[fontSetting] then
        return FONT_MAPPING[fontSetting]
    end
...
function EspObject:Update()
	...
	self.enabled = self.options.enabled and self.character and self.health > 0 and not
...
function EspObject:Render()
	...
	if visible.healthText.Visible then
		...
		healthText.Font = getFontValue(interface.sharedSettings.textFont);
	...
	if visible.name.Visible then
		...
		name.Font = getFontValue(interface.sharedSettings.textFont);
	...
	if visible.distance.Visible then
		...
		distance.Font = getFontValue(interface.sharedSettings.textFont);
	...
	if visible.weapon.Visible then
		...
		weapon.Font = getFontValue(interface.sharedSettings.textFont);
...
function ChamObject:Update()
	...
	local enabled = options.enabled and character and health > 0 and not
		(#interface.whitelist > 0 and not find(interface.whitelist, self.player.UserId));
...
function InstanceObject:Render()
	...
	if text.Visible then
		...
		text.Font = getFontValue(options.textFont);
